...
SuperPoint::SuperPoint(const std::string &model_path)
    : impl(new SuperPointImpl({.device = torch::cuda::is_available()
                                             ? torch::kCUDA
                                             : torch::kCPU})),
      point_min_score(0.015) {
  if (!fs::is_regular_file(model_path))
    throw fs::filesystem_error(
        "invalid model file", model_path,
...
SuperPoint::~SuperPoint() { delete impl; }

std::tuple<Eigen::MatrixX2d, Eigen::MatrixXd> SuperPoint::getFeatures(
  ...
  if (img.rows % cell != 0 || img.cols % cell != 0)
    throw std::runtime_error("image dimensions (" + std::to_string(img.cols) +
                             " x " + std::to_string(img.rows) +
                             ") must be multiple of cell size (" +
                             std::to_string(cell) + ")");

  // convert image to tensor
  const torch::Tensor input =
      torch::from_blob(img.data, {1, 1, img.rows, img.cols}).to(impl->device);
  ...
  // softmax
  tensor_semi = torch::nn::functional::softmax(tensor_semi.squeeze(0), 0)
                    .permute({1, 2, 0});
  ...
  const torch::Tensor heatmap_nms =
  ...
  const torch::Tensor mask_nms =
      torch::logical_and((heatmap > point_min_score), (heatmap == heatmap_nms));
  ...
  const torch::Tensor pts_norm =
      torch::roll(torch::nonzero(mask_nms).to(torch::kDouble) /
                      torch::tensor(input.sizes().slice(2, 2), impl->device),
                  1, 1);
  ...
  const Eigen::MatrixX2d coord =
      Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, 2, Eigen::RowMajor>>(
          pts_norm.cpu().contiguous().data_ptr<double>(), pts_norm.sizes()[0],
          pts_norm.sizes()[1]);

  // N x D keypoint descriptors {f_0, ..., f_n} with f as 1 x D row-vector
  const Eigen::MatrixXd descr = Eigen::Map<
      Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(
      desc.contiguous().data_ptr<double>(), desc.sizes()[0], desc.sizes()[1]);

#ifdef BENCHMARK
  const auto tend = sync();

  std::cout << "input load (ms): "
            << std::chrono::duration<float, std::milli>(tinf - tinput).count()
            << std::endl;
  std::cout << "inference (ms): "
            << std::chrono::duration<float, std::milli>(thm - tinf).count()
            << std::endl;
  std::cout << "heatmap (ms): "
            << std::chrono::duration<float, std::milli>(tkp - thm).count()
            << std::endl;
  std::cout << "keypoint (ms): "
            << std::chrono::duration<float, std::milli>(tformat - tkp).count()
            << std::endl;
  std::cout << "format (ms): "
            << std::chrono::duration<float, std::milli>(tend - tformat).count()
            << std::endl;
  std::cout << "TOTAL (ms): "
            << std::chrono::duration<float, std::milli>(tend - tinput).count()
            << std::endl;
#endif
